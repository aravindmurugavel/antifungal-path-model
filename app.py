...
st.markdown(
    ...
    <div class="app-header">
        ...
        <div class="subtext">
            AI-powered decision support system to provide patient-specific, evidence-based antifungal therapy recommendations, assisting the clinicians in seconds.
        </div>
    </div>
    """,
    unsafe_allow_html=True
...
with st.sidebar: 
    ...
    st.markdown(
        "**PATH (Personalized Antifungal Treatment for Humans)** is an AI-driven, open-source decision-support tool designed to generate patient-specific, evidence-based antifungal therapy recommendations. It addresses the challenge of rapidly selecting effective antifungal treatments, aiming to optimize therapy choices and improve clinical outcomes in fungal infections. "
    )
    ...
    st.markdown(
        "By intergation this evidence, PATH provides **immediate, evidence-based insights** to guide clinicians in selecting the most effective antifungal agents for immediate care."
    )
...
def display_combinations(combo_list):
    ...
    for d1, p1, d2, p2 in combo_list:
        ...
        # MODIFIED: Show sum as "success"
        pct_html = f'<div class="prob-pct" style="font-weight: 600; font-size: 0.9rem; white-space: nowrap;">= {pct(total_prob)} Success</div>'
...
if st.button(" ⚕ Predict Antifungal Treatment", type="primary"):
    ...
    with colA:
        st.markdown('<div class="section-title title-high">High Success (≥70%)</div>', unsafe_allow_html=True)
        display_results_by_class(high_conf, 'high')

    with colB:
        st.markdown('<div class="section-title title-mid">Moderate Success (30–70%)</div>', unsafe_allow_html=True)
        display_results_by_class(moderate_conf, 'mid')

    with colC:
        st.markdown('<div class="section-title title-low">Low Success (<30%)</div>', unsafe_allow_html=True)
    ...
    with col2_sum:
        ...
        **Important Disclaimer:**
        ...
        - **Confirmation of Antifungal Susceptibility is Mandatory:** The predictions generated by PATH model, do not account for local antimicrobial reistance patterns and are not a substitute for in-vitro Antifungal Susceptibility Test (AFST).
        - This tool is intended to assist clinicians in rapidly selecting evidence based antifungal therapy and does not replace expert judgment.
        """)
...
with st.expander("ℹ️ About this Application"):
    ...
    ### Developed by:
    - Mr. Aravind M | Ms. P.D.L. Sahithi | Ms. Sridevi Raghunathan | Prof. R. Jayapradha* (PI)
